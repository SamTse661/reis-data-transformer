...
export const appResultTransformer = (tableCode, processTimestamp) => {
...
const generateUUIDForData = (tableCode, data, processTimestamp) => {
  ...
  return data.map(row => {
    ...
    row.original_file_id = row.int_file_id;

    row.int_app_id = lookupExternalTableTd('int_002', processTimestamp, row.int_app_id);
    row.int_file_id = lookupExternalTableTd('int_001', processTimestamp, row.int_file_id);

    return row;
  });
};

export const appResultSqlGenerator = (tableCode, processTimestamp) => {
  ...
  delete transformedData[0].original_file_id;
