...
export const ubwFollowUpTransformer = (tableCode, processTimestamp) => {
...
const generateUUIDForData = (tableCode, data, processTimestamp) => {
  ...
  return data.map(row => {
    ...
    row.original_ubw_id = row.int_ubw_id;
    row.original_floor_unit_id = row.int_floor_unit_id;
    row.int_event_timeline_ubw_rectification = uuidv4();
    row.int_event_timeline_id_confirm = uuidv4();

    row.int_ubw_id = lookupExternalTableTd('int_018', processTimestamp, row.int_ubw_id);
    row.int_floor_unit_id = lookupExternalTableTd('int_016', processTimestamp, row.int_floor_unit_id);

    return row;
  });
};

export const ubwFollowUpSqlGenerator = (tableCode, processTimestamp) => {
  ...
  delete transformedData[0].original_ubw_id;
  ...
  for (const row of transformedData) {
    ...
    delete row.original_ubw_id;
    delete row.original_floor_unit_id;
    row.int_ubw_accepted_not = row.int_ubw_accepted_not == 1;
  }
