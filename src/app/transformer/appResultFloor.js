...
export const appResultFloorTransformer = (tableCode, processTimestamp) => {
...
const generateUUIDForData = (tableCode, data, processTimestamp) => {
  ...
  return data.map(row => {
    ...
    row.original_app_result_id = row.int_app_result_id;
    row.original_assessment_id = row.int_assessment_id;
    row.original_floor_unit_id = row.int_floor_unit_id;

    row.int_app_result_id = lookupExternalTableTd('int_028', processTimestamp, row.int_app_result_id);
...
export const appResultFloorSqlGenerator = (tableCode, processTimestamp) => {
  ...
  delete transformedData[0].original_app_result_id;
  delete transformedData[0].original_assessment_id;
  delete transformedData[0].original_floor_unit_id;
  ...
  for (const row of transformedData) {
    ...
    delete row.original_app_result_id;
