...
export const ubwTransformer = (tableCode, processTimestamp) => {
...
const generateUUIDForData = (tableCode, data, processTimestamp) => {
  ...
  return data.map(row => {
    ...
    row.original_property_bd_dlo_inspection_id = row.int_property_bd_dlo_inspection_id;

    row.int_property_bd_dlo_inspection_id = lookupExternalTableTd('int_017', processTimestamp, row.int_property_bd_dlo_inspection_id);

    return row;
  });
};

export const ubwSqlGenerator = (tableCode, processTimestamp) => {
  ...
  delete transformedData[0].original_property_bd_dlo_inspection_id;
  ...
  for (const row of transformedData) {
    ...
    delete row.original_property_bd_dlo_inspection_id;
    row.int_ubw_exist_not = row.int_ubw_exist_not == 1;
  }
